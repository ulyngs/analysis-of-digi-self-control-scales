...
draw_stacked_bar_chart_for_a_dimension_short <- function(some_dimension){
  plot <- upps_scores_long_only_top_factors %>% 
    filter(dimension == some_dimension) %>% 
    ggplot(aes(x = item, fill = fct_rev(score_numeric_reversed))) +
      geom_bar(position = "fill") +
      scale_fill_manual (values = drama_colors) +
...
walk(upps_dimensions, draw_stacked_bar_chart_for_a_dimension_short)
